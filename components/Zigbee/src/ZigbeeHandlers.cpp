...
static esp_err_t zb_cmd_door_lock_resp_handler(const esp_zb_zcl_door_lock_lock_door_message_t *message);
...
static esp_err_t zb_action_handler(esp_zb_core_action_callback_id_t callback_id, const void *message) {
  ...
  switch (callback_id) {
    ...
    case ESP_ZB_CORE_DOOR_LOCK_LOCK_DOOR_CB_ID:
      ret = zb_cmd_door_lock_resp_handler((esp_zb_zcl_door_lock_lock_door_message_t *)message);
      break;
...
static esp_err_t zb_cmd_door_lock_resp_handler(const esp_zb_zcl_door_lock_lock_door_message_t *message) {
  ...
  if (message->info.cluster != ESP_ZB_ZCL_CLUSTER_ID_DOOR_LOCK) {
    log_e("Received message to invalid handler, message was for: %d", message->info.cluster);
    return ESP_ERR_INVALID_ARG;
  }
  ...
  for (std::list<ZigbeeEP *>::iterator it = Zigbee.ep_objects.begin(); it != Zigbee.ep_objects.end(); ++it) {
    if (message->info.dst_endpoint == (*it)->getEndpoint()) {
      (*it)->zbDoorLockCmd(message);  //method zbDoorLockCmd must be implemented in specific EP class
    }
  }
